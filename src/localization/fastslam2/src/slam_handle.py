...
class FastSLAM2:
    ...
    def __init__(self):
        ...
        rospy.Subscriber('/lidar/cones', PointCloud2, self.camera_callback, queue_size=1)
    ...
    def process_map(self, observations: np.ndarray):
        t = time.time()
        # Since we assume that the car starts stopped, we keep the system
        # from updating its position for 5 iterations, to let the initial
        # landmarks converge.
        if self.i == 5:
